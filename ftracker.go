...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	v := meanSpeed(action, duration) * kmhInMsec
	h := height / cmInM
	result := ((walkingCaloriesWeightMultiplier*weight + (v*v/h)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return result
	// return (walkingCaloriesWeightMultiplier*weight + (v*v/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
